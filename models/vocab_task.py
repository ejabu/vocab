...
class vocab_task(osv.osv):
    ...
    def take_quiz(self, cr, uid, ids, context=None):
        ...
        # return
        dummy, view_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'account', 'view_account_chart') #in model name replace . with _ Example sale.order become sale_order
        import ipdb; ipdb.set_trace()
        return {
            'name':"HEHE",#Name You want to display on wizard
            'view_mode': 'form',
            'view_id': view_id,
            'view_type': 'form',
